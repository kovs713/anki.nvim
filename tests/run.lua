...
test("dashboard render lines handle empty state", function()
	...
	assert_true(rendered:find("%[q%] close") ~= nil, "missing bracketed close key")
end)
...
test("dashboard render shows valid Onigiri data", function()
	...
	local rendered = table.concat(lines, "\n")
	assert_true(rendered:find("🍙 Onigiri Companion", 1, true), "missing companion title")
	assert_true(rendered:find("Lv%. 4") ~= nil, "missing level")
	assert_true(rendered:find("Activity", 1, true), "missing activity section")
	assert_true(rendered:find("█", 1, true), "missing activity heatmap")
	assert_true(not rendered:find("430 XP", 1, true), "overview should not show xp")
	assert_true(not rendered:find("🪙 12", 1, true), "overview should not show coins")
	assert_true(not rendered:find("Anki collection", 1, true), "must not include anki stats")
	config.setup()
end)
...
test("stats view renders only Onigiri data", function()
	...
	local rendered = table.concat(lines, "\n")
	assert_true(rendered:find("🍙 Onigiri · Stats", 1, true), "missing Onigiri stats title")
	assert_true(rendered:find("RESTAURANT  Lv%. 4") ~= nil, "missing restaurant level")
	assert_true(rendered:find("Daily Specials", 1, true), "missing daily specials header")
	assert_true(rendered:find("Achievements", 1, true), "missing achievements header")
...
test("review_activity_by_day parses AnkiConnect heatmap rows", function()
	local anki = require("anki_review.anki")
	with_anki_stubs({
		request = function(action)
			eq(action, "getNumCardsReviewedByDay")
			return {
				{ "2026-05-31", 58 },
				{ "2026-05-30", 108 },
			}, nil
		...
		local activity, err = anki.review_activity_by_day()
		eq(err, nil)
		eq(#activity.items, 2)
		eq(activity.items[1].date, "2026-05-30")
		eq(activity.items[1].count, 108)
	end)
end)
